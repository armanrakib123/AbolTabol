...
export default function Page() {
  ...
  const handle_login_submit = async (e) => {
    ...
    try {
      const response = await signIn("patient-login", {
        email,
        password,
        redirect: false,
      });
      ...
      if (response?.ok) {
        ...
        const errMsg = response?.error || "Failed to login";
        toast.error(errMsg);
      }
    } catch (error) {
      toast.dismiss(loadingToastId);
      ...
      <div className={`${styles.localWrapper}`}>
        ...
        <div className="bg-gradient-to-r from-[#037aa5] to-[var(--mainColor)] font-normal min-h-screen  place-content-center overflow-hidden">
          ...
          <div className="flex justify-center">
            <div className={`relative w-[30rem] sm:w-[30rem] h-[40rem] ${animation}`}>
